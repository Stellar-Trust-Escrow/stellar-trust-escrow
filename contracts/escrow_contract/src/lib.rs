...
impl ContractStorage {
    ...
    fn load_escrow(env: &Env, escrow_id: u64) -> Result<EscrowState, EscrowError> {
        ...
        let timelock_release_at = match &meta.timelock {
            OptionalTimelock::Some(tl) => Some(tl.start_ledger + tl.duration_ledger),
...
impl EscrowContract {
    ...
    // ── Timelock claim & early-release multi-sig ──────────────────────────────

    /// Allows the contractor (freelancer) to claim funds after the timelock expires.
    pub fn claim_after_timelock(env: Env, escrow_id: u64) -> Result<(), EscrowError> {
        let escrow = ContractStorage::load_escrow(&env, escrow_id)?;
        escrow.freelancer.require_auth();
        let release_at = escrow.timelock_release_at.ok_or(EscrowError::E11)?;
        if env.ledger().timestamp() < release_at {
            return Err(EscrowError::TimelockNotExpired);
        }
        // TODO: transfer funds and update escrow state to Released/Completed
        Ok(())
    }

    /// Early release with multi-sig override.
    /// TODO: implement actual Ed25519 verification with stored public keys.
    pub fn early_release(
        ...
        let _escrow = ContractStorage::load_escrow(&env, escrow_id)?;
        let _ = (contractor_sig, client_sig);
        // TODO: verify Ed25519 signatures with stored public keys and transfer funds
        Ok(())
    }
