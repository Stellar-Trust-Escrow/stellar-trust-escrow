...
import {
  ...
  computeSHA256,
} from '../../../hooks/useEscrowDraft';

const STEPS = ['Parties', 'Milestones', 'Terms & Timelock', 'Review & Sign'];

export default function CreateWizard() {
  const { draft, dispatch, milestonesTotal, isValidStep1, isValidStep2, isValidStep3 } =
    useEscrowDraft();
  ...
  return (
    <div className="create-wizard" role="region" aria-label="Escrow Creation Wizard">
      <div className="wizard-progress" aria-live="polite">
        {STEPS.map((label, i) => (
          <div key={i} className={`wizard-step ${i + 1 === draft.step ? 'active' : ''}`}>
      ...
      {draft.step === 1 && (
        <fieldset>
          ...
          <input
            ...
            onChange={(e) =>
              dispatch({ type: 'SET_FIELD', field: 'counterparty', value: e.target.value })
            }
            placeholder="G..."
          />
          ...
          <input
            ...
            value={draft.arbiter}
            onChange={(e) =>
              dispatch({ type: 'SET_FIELD', field: 'arbiter', value: e.target.value })
            }
            placeholder="G..."
          />
        </fieldset>
      ...
      {draft.step === 2 && (
        <fieldset>
          ...
          {draft.milestones.length < 10 && (
            <button type="button" onClick={() => dispatch({ type: 'ADD_MILESTONE' })}>
              + Add Milestone
            </button>
          )}
      ...
      {draft.step === 3 && (
        <fieldset>
          ...
          <label>
            <input
              type="checkbox"
              checked={draft.timelock}
              onChange={(e) =>
                dispatch({ type: 'SET_FIELD', field: 'timelock', value: e.target.checked })
              }
            />{' '}
            Enable Timelock
          </label>
          <label>
            <input
              ...
              onChange={(e) =>
                dispatch({ type: 'SET_FIELD', field: 'sequential', value: e.target.checked })
              }
            />{' '}
      ...
      {draft.step === 4 && (
        <div>
          <h3>Review & Sign</h3>
          <p>
            <strong>Counterparty:</strong> {draft.counterparty}
          </p>
          <p>
            <strong>Arbiter:</strong> {draft.arbiter}
          </p>
          <p>
            <strong>Milestones:</strong> {draft.milestones.length} ({milestonesTotal} XLM)
          </p>
          <p>
            <strong>Deadline:</strong> {draft.deadline}
          </p>
          <p>
            <strong>Timelock:</strong> {draft.timelock ? 'Yes' : 'No'}
          </p>
          <button type="button" onClick={() => alert('Transaction would be signed via Freighter')}>
            Sign & Create
          </button>
        </div>
      )}
      ...
      <div className="wizard-nav">
        ...
        {draft.step < 4 && (
          <button
            ...
            onClick={nextStep}
            disabled={
              draft.step === 1 ? !isValidStep1 : draft.step === 2 ? !isValidStep2 : !isValidStep3
            }
          >
            Next
          </button>
