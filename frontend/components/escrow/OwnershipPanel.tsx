  ...
  return (
    <section className="rounded-xl border border-gray-200 dark:border-gray-700 p-4 space-y-3">
      ...
      <div className="flex items-center gap-2 text-sm">
        ...
        {data.currentOwner ? (
          <>
            <span title={data.currentOwner} className="font-mono text-gray-800 dark:text-gray-200">
        ...
        <details className="text-xs">
          ...
          <ul className="mt-2 space-y-1">
            {data.transferLog.map((entry) => (
              ...
              >
                <span className="font-mono" title={entry.fromOwner}>
                  {truncate(entry.fromOwner)}
                </span>
                <span>→</span>
                <span className="font-mono" title={entry.toOwner}>
                  {truncate(entry.toOwner)}
                </span>
