...
const WARNINGS = new Set(['CC-BY-4.0', 'CC-BY-3.0']);
...
const exceptionsConfig = fs.existsSync(exceptionsPath)
...
const isException = (name, version, licenseId) => {
  return exceptionsConfig.exceptions.some(
    (ex) =>
      ex.name === name &&
  ...
  for (const licObj of component.licenses) {
    ...
    if (ALLOWED.has(licenseId)) {
      ...
      if (isException(name, version, licenseId)) {
        ...
        console.warn(
          `[WARNING] Acceptable for docs-only, but verify: ${name}@${version} - ${licenseId}`,
        );
        hasWarnings = true;
      }
    } else {
